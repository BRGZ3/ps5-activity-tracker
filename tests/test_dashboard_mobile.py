...
class DashboardMobileContractTests(unittest.TestCase):
    def test_dashboard_has_mobile_layout_contract(self):
        ...
        self.assertIn('href="library.html?v=', html)
        self.assertIn('id="libraryLink"', html)
        self.assertIn('fetch(`/summary.json?', html)
    ...
    def test_library_page_contract(self):
        html = (ROOT / "dashboard/library.html").read_text(encoding="utf-8")
        self.assertIn('name="viewport"', html)
        self.assertIn('id="games"', html)
        self.assertIn('data-playlog-ui=', html)
        self.assertIn('UI 1.49.0', html)
        self.assertIn('id="search"', html)
        self.assertIn('XMLHttpRequest', html)
        self.assertIn('/api/installed-games?v=', html)
        self.assertNotIn('/summary.json?v=', html)
        self.assertIn('installed===true', html)
        self.assertIn('installed===false', html)
        self.assertIn('id="installedHeading"', html)
        self.assertIn('id="historyGames"', html)
        self.assertIn('/api/game-icon?title_id=', html)
        self.assertNotIn('loading="lazy"', html)
